...
function resolveConflict(name, a, b) {
    ...
    if (a.type === 'concrete' && b.type === 'concrete') {
        if (b.resolvedDirectory !== a.resolvedDirectory) {
            ...
            throw new Error(`${name} can be either ${a.resolvedDirectory} (v${aVersion})‚ or ${b.resolvedDirectory} (v${bVersion})`);
        }
        return a;
    }
    ...
    if (a.type === 'concrete' && b.type === 'symbolic') {
        ...
        if (!semver.satisfies(concreteVersion, b.versionRange, { includePrerelease: true })) {
            throw new Error(`${name} expected to match ${b.versionRange} but found ${concreteVersion} at ${a.resolvedDirectory}`);
        }
        return a;
    }
...
function fmtSource(loc) {
    return loc ? (0, snippet_1.formatLocation)(loc.location) : '** should never happen**';
}
