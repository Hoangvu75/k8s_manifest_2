...
function loadProjectInfo(projectRoot) {
    ...
    const projectInfo = {
        ...
        bundleDependencies,
        targets: {
            ...validateTargets(_required(pkg.jsii, 'The "package.json" file must specify the "jsii" attribute').targets),
            js: { npm: pkg.name },
        },
        metadata,
...
function validateTargets(targets) {
    ...
    for (const [language, config] of Object.entries(targets)) {
        if (!(language in VALID_TARGET_KEYS)) {
            throw new utils_1.JsiiError(`Unknown target language: ${language}`);
        }
        if (typeof config !== 'object' || config === null) {
            throw new utils_1.JsiiError(`jsii.targets.${language} must be an object`);
        }
        const validKeys = VALID_TARGET_KEYS[language];
        for (const key of Object.keys(config)) {
            if (!validKeys.includes(key)) {
                throw new utils_1.JsiiError(`Unknown key in jsii.targets.${language}: ${key}`);
            }
        }
    }
    ...
    if (targets.go &&
        ...
        if (!isIdentifier(targets.go.packageName)) {
            throw new utils_1.JsiiError(`jsii.targets.go.packageName contains non-identifier characters: ${targets.go.packageName}`);
        }
    }
    if (targets.dotnet &&
        ...
        if (!targets.dotnet.namespace.split('.').every(isIdentifier)) {
            throw new utils_1.JsiiError(`jsii.targets.dotnet.namespace contains non-identifier characters: ${targets.dotnet.namespace}`);
        }
    }
    if (targets.java &&
        ...
        if (!targets.java.package.split('.').every(isIdentifier)) {
            throw new utils_1.JsiiError(`jsii.targets.java.package contains non-identifier characters: ${targets.java.package}`);
        }
    }
    if (targets.python &&
        ...
        if (!targets.python.module.split('.').every(isIdentifier)) {
            throw new utils_1.JsiiError(`jsii.targets.python.module contains non-identifier characters: ${targets.python.module}`);
        }
    }
    return targets;
    /**
...
class DependencyResolver {
    ...
    loadAssemblyAndRecurse(jsiiFile) {
        ...
        const assembly = (0, spec_1.loadAssemblyFromFile)(jsiiFile, true, helpers_1.ASSEMBLY_FEATURES_SUPPORTED);
